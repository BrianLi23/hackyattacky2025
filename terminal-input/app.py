...
def get_products_from_db() -> List[Dict]:
    """Get all products from database (uses ProductManager class)"""
    return product_manager.get_all_products()

def get_product_by_id(product_id: int) -> Optional[Dict]:
    """Get single product by ID from database (uses ProductManager class)"""
...
def health_check():
    ...
    return jsonify({'status': 'healthy', 'message': 'EcoTech API is running'})
...
if __name__ == '__main__':
    ...
    if init_database():
        ...
        print("Database initialization failed, will use fallback data only")
